...
@jax.jit
def _rz_1(theta):
    return jnp.diag(jnp.exp(jnp.array([-0.5j * theta, 0.5j*theta, -0.5j*theta, 0.5j*theta])))
...
def _complex_sort(x):
    ...
    return jnp.sort(x.real + x.imag * 1e5)
...
def _v2_angles(evals):
    ...
    abs_angles = jnp.abs(angles)
    r, s = jax.lax.cond(
        jnp.isclose(abs_angles[0], abs_angles[1]),
        lambda abs_angles: (abs_angles[0], abs_angles[2]),
        lambda abs_angles: (abs_angles[0], abs_angles[1]),
        abs_angles,
    )
    return (r + s) / 2, (r - s) / 2
...
def _prop_v2(u):
    ...
    t = jnp.diag(_gamma(u.T).T)
    psi = jnp.arctan2(jnp.sum(t).imag, (t[0] + t[3] - t[1] - t[2]).real)
...
def _prop_iv3(u, v):
    ...
    pi_u = delta_u.real * jnp.pi**2 + delta_u.imag / jnp.pi**2
    pi_v = delta_v.real * jnp.pi**2 + delta_v.imag / jnp.pi**2
...
@jax.jit
def _compute_decomp_data(arr):
    psi, theta, phi = _prop_v2(arr)
    _u = arr @ _cnot @ _rz_1(psi) @ _cnot
    v = _cnot @ _rx_rz(theta, phi) @ _cnot
    a, b, c, d = _prop_iv3(_u, v)
    return psi, theta, phi, a, b, c, d
...
def asymmetric_two_qubit_decomp(u, wires):
    ...
    psi, theta, phi, a, b, c, d = _compute_decomp_data(jnp.array(u_mod))
