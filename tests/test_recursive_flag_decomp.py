...
class TestMuxOps:
    ...
    @pytest.mark.parametrize(
        "ops", [[qml.CNOT([0, 1])], [qml.X(0)], [qml.CNOT([0, 1]), qml.CY([1, 5])]]
    )
    ...
    def test_with_single_mux_flag(self, k, num_controls):
        ...
        ops = [
            [MultiplexedFlag(*thetas[2 * i : 2 * i + 2], list(range(k + 1)))]
    ...
    def test_with_mixed_ops(self, k, num_controls):
        ...
        assert all(
            isinstance(new_op, type(op0)) for new_op, op0 in zip(new_ops, ops[0], strict=True)
        )
...
class TestMuxMultiQubitDecomp:
    ...
    @pytest.mark.parametrize(
        "num_controls, num_targets", [(1, 3), (1, 4), (2, 3), (2, 4), (3, 3), (4, 3)]
    )
    @pytest.mark.parametrize("n_b", [1, 2])
...
class TestRecursiveFlagDecompCliffRz:
    ...
    def test_main_usage(self, seed, num_targets, n_b, selective_demux):
        ...
        assert np.allclose(rec_mat, V)
        assert all(
            isinstance(op, (qml.RZ, qml.RY, qml.CZ, qml.CNOT)) for op in ops
        ), f"{set(type(op) for op in ops)}"
        num_rots = sum(isinstance(op, (qml.RZ, qml.RY)) for op in ops)
        n = num_targets
